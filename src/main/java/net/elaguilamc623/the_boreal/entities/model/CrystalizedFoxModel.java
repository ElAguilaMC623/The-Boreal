...
@OnlyIn(Dist.CLIENT)
public class CrystalizedFoxModel<T extends CrystalizedFox> extends AgeableListModel<T> {
    ...
    public CrystalizedFoxModel(ModelPart root) {
    ...
    public void prepareMobModel(T crystalizedFox, float limbSwing, float limbSwingAmount, float partialTicks) {
        ...
        this.head.yRot = 0.0F;
        this.head.zRot = crystalizedFox.getHeadRollAngle(partialTicks);
        ...
        if (crystalizedFox.isCrouching()) {
            this.body.xRot = 1.6755161F;
            float f = crystalizedFox.getCrouchAmount(partialTicks);
            this.body.setPos(0.0F, 16.0F + crystalizedFox.getCrouchAmount(partialTicks), -6.0F);
            this.head.setPos(-1.0F, 16.5F + f, -3.0F);
            this.head.yRot = 0.0F;
        } else if (crystalizedFox.isSleeping()) {
        ...
        } else if (crystalizedFox.isSitting()) {
    ...
    public void setupAnim(T crystalizedFox, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!crystalizedFox.isSleeping() && !crystalizedFox.isFaceplanted() && !crystalizedFox.isCrouching()) {
            this.head.xRot = headPitch * ((float)Math.PI / 180F);
            this.head.yRot = netHeadYaw * ((float)Math.PI / 180F);
        }

        if (crystalizedFox.isSleeping()) {
            this.head.xRot = 0.0F;
            this.head.yRot = -2.0943952F;
        ...
        if (crystalizedFox.isFaceplanted()) {
            float f1 = 0.1F;
